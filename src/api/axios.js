...
import { mockAchievements } from "../utils/mockAchievements.js";
...
const route = async (method, url, data, params) => {
  ...
  // ─── AI Coach Init ──────────────────────────────────────────────────
  if (method === "GET" && url === "/ai/coach-init") {
    const activeHabits = state.habits.filter((h) => !h.isArchived);
    const today = todayKey();
    const todayLogs = state.logs.filter((l) => l.completedDate === today);

    // Compute max current streak across habits
    let maxStreak = 0;
    for (const h of activeHabits) {
      const hLogs = state.logs
        ...
        .sort()
        .reverse();
      const s = mockStreak(hLogs);
      if (s.current > maxStreak) maxStreak = s.current;
    }

    // Weekly rate
    const weekAgo = format(subDays(new Date(), 6), "yyyy-MM-dd");
    const weekLogs = state.logs.filter(
      (l) => l.completedDate >= weekAgo && l.completedDate <= today
    );
    const weekTotal = activeHabits.length * 7;
    const weekRate = weekTotal
      ? Math.round((weekLogs.length / weekTotal) * 100)
      : 0;
    ...
    return {
      ...
      greeting: `Hey ${state.user.name.split(" ")[0]}! I've looked at your last 14 days of data. You're on a great streak — but I noticed you tend to skip your evening habits on weekends. Want to talk about what's getting in the way?`,
    };
  }

  // ─── Achievements ───────────────────────────────────────────────────
  if (method === "GET" && url === "/achievements")
